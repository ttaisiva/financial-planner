export async function createTablesIfNotExist(connection) {
  ...
  CREATE TABLE IF NOT EXISTS user_scenario_info (
    ...
    user_birth_year INT CHECK (user_birth_year >= 1900 AND user_birth_year <= YEAR(CURDATE())),,
    ...
    spouse_birth_year INT CHECK (spouse_birth_year >= 1900 AND spouse_birth_year <= YEAR(CURDATE())),
