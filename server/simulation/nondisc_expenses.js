...
import { calculateExpenseAmount, getExpenseWithdrawalStrategy} from "./disc_expenses.js";
/**
...
export async function payNonDiscExpenses(
    ...
    const nonDiscretionaryExpenses = await getNonDiscretionaryExpenses(scenarioId);
    console.log("Non-discretionary expenses fetched:", nonDiscretionaryExpenses);

    // Filter active non-discretionary events
    const activeEvents = await filterActiveNonDiscretionaryEvents(nonDiscretionaryExpenses, currentSimulationYear);
    ...
    for (const expense of activeEvents) {
        const expenseAmount = calculateExpenseAmount(expense, currentSimulationYear, inflationRate);
        console.log(`Attempting to pay non-discretionary expense: ${expense.name}, amount: ${expenseAmount}`);
        ...
        if (runningTotals.cashInvestment >= expenseAmount) {
            ...
            remainingWithdrawal = Number(expenseAmount) - Number(runningTotals.cashInvestment);
            ...
            for (const investment of strategyInvestments) {
                ...
                if (investment.taxStatus === "non-retirement") {
                    ...
                    const purchasePriceID = runningTotals.purchasePrices[String(investment.id)];
...
async function getNonDiscretionaryExpenses(scenarioId) {
    console.log(`Fetching non-discretionary expenses for scenario ID: ${scenarioId}`);
 ...
 */
async function filterActiveNonDiscretionaryEvents(nonDiscretionaryEvents, currentSimulationYear) {
    const activeEvents = [];
