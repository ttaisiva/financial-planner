...
export function calculateSuccessProbability(
  ...
  return Object.entries(yearlySuccess).map(
    ([year, { successCount, totalCount }]) => ({
      year: Number(year),
...
export function ShadedLineChart({
  label,
  allSimulationResults,
  financialGoal,
}) {
      ...
      simulation.forEach((yearlyResult) => {
        ...
        if (selectedOption === "allInvestments") {
          ...
          if (selectedOption === "expenses") {
            ...
            console.log(
              "Adding taxes to yearly expenses: ",
              yearlyResult.taxes
            );
        ...
        // Calculate the percentage of discretionary expenses incurred for the year
          ...
          console.log(
            ...
            yearlyResult.actualDiscExpenses,
            yearlyResult.expenses
          );
          ...
          const desired =
            yearlyResult.expenses
              ...
              .reduce(
                (sum, expense) => Number(sum) + Number(expense.adjustedAmount),
                0
              ) || 1; // Sum initial amounts of discretionary expenses

          const percentage = (incurred / desired) * 100; // Calculate percentage
          console.log(
            `Year: ${year}, Incurred: ${incurred}, Desired: ${desired}, Percentage: ${percentage}`
          );
...
export function StackedBarChart({
  ...
  Object.entries(processedData).forEach(([year, categories]) => {
    ...
    Object.entries(categories).forEach(([category, value]) => {
      if (value >= aggregationThreshold) {
        ...
        console.log(
          `Category ${category} with value ${value} is below aggregation threshold of ${aggregationThreshold}. Moving to "Other".`
        );
  ...
  const datasets = Array.from(allCategories).map((category) => ({
    ...
    backgroundColor: `rgba(${Math.floor(Math.random() * 255)}, ${Math.floor(
      Math.random() * 255
    )}, ${Math.floor(Math.random() * 255)}, 0.5)`, // Random color for each category
  }));
